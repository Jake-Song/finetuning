...
def dry_run(cfg: TrainConfig, resume_from_checkpoint: str | None = None) -> None:
    ...
    public_implemented = len(PUBLIC_IFEVAL_INSTRUCTION_IDS & CHECKERS.keys())
    missing_public = sorted(PUBLIC_IFEVAL_INSTRUCTION_IDS - CHECKERS.keys())
    dataset_missing_public = sorted(cid for cid in counts if cid in PUBLIC_IFEVAL_INSTRUCTION_IDS and cid not in CHECKERS)
    ...
    print(f"  public IFEval coverage: {public_implemented}/{len(PUBLIC_IFEVAL_INSTRUCTION_IDS)}")
    if missing_public:
        print(f"  missing public ids: {missing_public}")
    if dataset_missing_public:
        print(f"  unsupported public ids in dataset: {dataset_missing_public}")
