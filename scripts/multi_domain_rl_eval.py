...
from multi_domain_rl_train import TrainConfig, load_multi_domain_dataset, reward_fn
...
def score_example(example: dict, completion: str) -> float:
    return reward_fn(
        completions=[completion],
        expected_answer=[example.get("expected_answer", "")],
        category=[example.get("category", "")],
        ground_truth=[json.loads(example.get("ground_truth", "[]"))],
    )[0]


def main():
    default_cfg = TrainConfig()
    parser = argparse.ArgumentParser(description="Evaluate a trained model on multi-domain RL tasks")
    ...
    examples = load_eval_examples(ds_name, ds_config, cfg.max_prompt_length, tokenizer_path, eval_size)
    ...
    for i, ex in enumerate(examples):
        ...
        for _ in range(num_samples):
            ...
            reward = score_example(ex, completion)
        ...
        result = {
            ...
            "ground_truth": json.loads(ex.get("ground_truth", "[]")),
    ...
    elapsed = time.time() - started
    print(f"\n{'='*60}")
    print(f"Multi-Domain RL Results ({ds_name}/{ds_config}): {model_path}")
    print(f"{'='*60}")
