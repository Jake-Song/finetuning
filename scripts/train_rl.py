...
from tasks.ifrl import IFRL
from utils.common import compute_init, compute_cleanup, print0, get_base_dir, DummyWandb, autodetect_device_type

# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Reinforcement learning on IF-RL")
# Logging
...
# Training horizon
parser.add_argument("--num-epochs", type=int, default=1, help="number of epochs over IF-RL")
...
full_task = IFRL(subset="IF-RL", split="train")
# take the last N examples as val, rest as train
full_task_size = full_task.num_examples()
val_size = full_task_size // 10
train_task = full_task[:full_task_size - val_size]   # need __getitem__ slice support
val_task   = full_task[full_task_size - val_size:]
